...
C
C         shortwave CH4 perturbation multiplier
C         CH4 mult=1 when prof amount = 1.05 * ref amount
          IF (LCH4) THEN
            CH4MLT(L)=20.0*( PMAMNT(L) - FIXMUL(L)*RMAMNT(L) )/
     $         RMAMNT(L)
C           Ignore changes in CH4 less than ~0.3%
            IF (ABS(CH4MLT(L)) .LT. 1E-2) CH4MLT(L)=0.0
          ELSE
             CH4MLT(L)=0.0
          ENDIF
C
