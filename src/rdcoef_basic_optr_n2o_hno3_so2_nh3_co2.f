...
C    coefficients.
C    22 Jan 2019 C Hepplewhite  Added NH3
!END====================================================================
...
C      ---------------------
C      Read NH3 perturb coefs - placeholder while no coef file.
C      ---------------------
...
C         Keep the data if the current channel is on the list
       ...
       ENDDO
       CLOSE(IOUN)
       WRITE(6,'(A,1X,I4)') 'Completed rdcoef NH3. no chans: ',J
C
C - these lines used as placeholder when no ceofficients are available.
C    J=1
C    DO I=1,MXCHNH
