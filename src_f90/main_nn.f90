...
!  For tunmlt
! --------------------------------------------------
! integer, dimension(MXCNTE) :: CLISTN                 ! non-LTE channel list
real(4), dimension(NNCOEF,MXCNTE) :: COEFN           ! non-LTE coefficients
...
! for calxnte_nn
! ----------------------------------------
real(4), dimension(4) :: PRDNTE           ! sunang, view ang, temp(z1), temp(z2)
...
!      View angle should be within a few degrees of scan angle
 ...
 1030     FORMAT('Error! Profile',I5, &
       ...
       ENDIF
     ENDIF
   write(6,*) 'main: completed satzen satang to view ang'
! --------------------------
! truncate view angle
! --------------------------
...
!
     ...
     write(6,*) 'main: completed view and sun angle calcs'
!
!  -----------------------------------
...
!       No sun; set the sun surface-to-space trans to zero
        ...
        ENDDO
     ENDIF
     write(6,*) 'main: completed caltN after if dosun'
!
!  -----------------------------------------------------
...
!
     if (COFNTE) then
! assign PRDNTE values from current profile
! limit max sunag to fitting range (currently 120-deg)
       PRDNTE(1) = cos(min(sunang,120.0)*DEG2RAD)
!!!         PRDNTE(1) = cos( SACONV(min(sunang,120.0), ALT(1)) )
       PRDNTE(2) = cos(SATZEN*DEG2RAD)       
       PRDNTE(3) = (TEMP(1) + TEMP(2) + TEMP(3) + TEMP(4) + TEMP(5))/5.0 ! mean(tprof(1:5,:),1);
       PRDNTE(3) = PRDNTE(3)/224.4438  
       PRDNTE(4) = (TEMP(6) + TEMP(7) + TEMP(8) + TEMP(9))/4.0           ! mean(tprof(6:9,:),1);
       PRDNTE(4) = PRDNTE(4)/257.1381
! -------------------------
!  calculate nonLTE
! -------------------------
       CALL calxnte(INDCHN,CLISTN,FCHANN, IP_YMAX,IP_YMIN,IP_XMAX,IP_XMIN, &
...
!  Close the RTP files
!  -------------------
 9999  write(6,*) 'main: closing rtp files'
