...
LCO2,  LN2O,  LSO2, LNH3,  LHDO  LHNO3, LCO2PM, LCH4,  &
      ...
      WPRED1,WPRED2,WPRED3,WPRED4,WPRED5,WPRED6,WPRED7,  &
      OPRED1,OPRED2,       OPRED4,OPRED5,OPRED6,OPRED7,  &
      MPRED3,CPRED4,TRCPRD,CO2MLT,SO2MLT,HNOMLT,N2OMLT,NH3MLT,HDOMLT, CH4MLT)
!      =================================================================
...
!-----------------------------------------------------------------------
  ...
  REAL, INTENT(IN)                         :: RTEMP(MAXLAY)
  REAL, INTENT(IN)                         :: RFAMNT(MAXLAY)
  REAL, INTENT(IN)                         :: RWAMNT(MAXLAY)
  REAL, INTENT(IN)                         :: ROAMNT(MAXLAY)
  REAL, INTENT(IN)                         :: RCAMNT(MAXLAY)
  REAL, INTENT(IN)                         :: RMAMNT(MAXLAY)
  REAL, INTENT(IN)                         :: RSAMNT(MAXLAY)
  REAL, INTENT(IN)                         :: RHAMNT(MAXLAY)
  REAL, INTENT(IN)                         :: RNAMNT(MAXLAY)
  REAL, INTENT(IN)                         :: RAAMNT(MAXLAY)
  REAL, INTENT(IN)                         :: PTEMP(MAXLAY)
  REAL, INTENT(IN)                         :: PFAMNT(MAXLAY)
  REAL, INTENT(IN)                         :: PWAMNT(MAXLAY)
  REAL, INTENT(IN)                         :: POAMNT(MAXLAY)
  REAL, INTENT(IN)                         :: PCAMNT(MAXLAY)
  REAL, INTENT(IN)                         :: PMAMNT(MAXLAY)
  REAL, INTENT(IN)                         :: PSAMNT(MAXLAY)
  REAL, INTENT(IN)                         :: PHAMNT(MAXLAY)
  REAL, INTENT(IN)                         :: PNAMNT(MAXLAY)
  REAL, INTENT(IN)                         :: PAAMNT(MAXLAY)
  REAL, INTENT(IN)                         :: PRES(MAXLAY)
  REAL, INTENT(IN)                         :: SECANG(MAXLAY)
  REAL, INTENT(IN OUT)                     :: ALAT
  REAL, INTENT(IN)                         :: FX(MAXLAY)
  REAL, INTENT(IN)                         :: DZREF(MAXLAY)
  LOGICAL, INTENT(IN OUT)                  :: LCO2
  LOGICAL, INTENT(IN OUT)                  :: LN2O
  LOGICAL, INTENT(IN OUT)                  :: LSO2
  LOGICAL, INTENT(IN OUT)                  :: LNH3
  NO TYPE, INTENT(IN OUT)                  :: LHDO  LHNO
    LOGICAL, INTENT(IN OUT)                  :: LCO2PM
    LOGICAL, INTENT(IN OUT)                  :: LCH4
    REAL, INTENT(IN OUT)                     :: CO2PPM
    REAL, INTENT(OUT)                        :: CO2TOP
    REAL, INTENT(OUT)                        :: FIXMUL(MAXLAY)
    REAL, INTENT(OUT)                        :: CONPRD( N1CON,MAXLAY)
    REAL, INTENT(OUT)                        :: DPRED(  NHDO MAXLAY)
      REAL, INTENT(OUT)                        :: FPRED1( N1FIX,MAXLAY)
      REAL, INTENT(OUT)                        :: FPRED2( N2FIX,MAXLAY)
      REAL, INTENT(OUT)                        :: FPRED3( N3FIX,MAXLAY)
      REAL, INTENT(OUT)                        :: FPRED4( N4FIX,MAXLAY)
      REAL, INTENT(OUT)                        :: FPRED5( N5FIX,MAXLAY)
      REAL, INTENT(OUT)                        :: FPRED6( N6FIX,MAXLAY)
      REAL, INTENT(OUT)                        :: FPRED7( N7FIX,MAXLAY)
      REAL, INTENT(OUT)                        :: WPRED1( N1H2O,MAXLAY)
      REAL, INTENT(OUT)                        :: WPRED2( N2H2O,MAXLAY)
      REAL, INTENT(OUT)                        :: WPRED3( N3H2O,MAXLAY)
      REAL, INTENT(OUT)                        :: WPRED4( N4H2O,MAXLAY)
      REAL, INTENT(OUT)                        :: WPRED5( N5H2O,MAXLAY)
      REAL, INTENT(OUT)                        :: WPRED6( N6H2O,MAXLAY)
      REAL, INTENT(OUT)                        :: WPRED7( N7H2O,MAXLAY)
      REAL, INTENT(OUT)                        :: OPRED1(  N1O3,MAXLAY)
      REAL, INTENT(OUT)                        :: OPRED2(  N2O3,MAXLAY)
      REAL, INTENT(OUT)                        :: OPRED4(  N4O3,MAXLAY)
      REAL, INTENT(OUT)                        :: OPRED5(  N5O3,MAXLAY)
      REAL, INTENT(OUT)                        :: OPRED6(  N6O3,MAXLAY)
      REAL, INTENT(OUT)                        :: OPRED7(  N7O3,MAXLAY)
      REAL, INTENT(OUT)                        :: MPRED3( N3CH4,MAXLAY)
      REAL, INTENT(OUT)                        :: CPRED4(  N4CO,MAXLAY)
      REAL, INTENT(OUT)                        :: TRCPRD(NTRACE,MAXLAY)
      REAL, INTENT(OUT)                        :: CO2MLT(MAXLAY)
      REAL, INTENT(OUT)                        :: SO2MLT(MAXLAY)
      REAL, INTENT(OUT)                        :: HNOMLT(MAXLAY)
      REAL, INTENT(OUT)                        :: N2OMLT(MAXLAY)
      REAL, INTENT(OUT)                        :: NH3MLT(MAXLAY)
      REAL, INTENT(OUT)                        :: HDOMLT(MAXLAY)
      REAL, INTENT(OUT)                        :: CH4MLT(MAXLAY)
      IMPLICIT NONE
...
!         ---------------
          IF (DEBUG) THEN
            IF(L == 96) WRITE(6,'(A,X,I4,X,F6.2)') 'calpar: L,HDOFCT ',L,HDOFCT
          END IF
...
!         shortwave CH4 perturbation multiplier
!         CH4 mult=1 when prof amount = 1.05 * ref amount
          IF (LCH4) THEN
            CH4MLT(L)=20.0*( PMAMNT(L) - FIXMUL(L)*RMAMNT(L) )/ RMAMNT(L)
!           Ignore changes in CH4 less than ~0.3%
            IF (ABS(CH4MLT(L)) < 1E-2) CH4MLT(L)=0.0
          ELSE
            CH4MLT(L)=0.0
          END IF
...
!               Ignore changes in CO2 of less than ~0.03%
              IF (ABS(CO2MLT(L)) < 1E-2) CO2MLT(L)=0.0
            END IF
          ELSE
...
!            Ignore changes in N2O less than ~0.3%
            IF (ABS(N2OMLT(L)) < 1E-2) N2OMLT(L)=0.0
          ELSE
            N2OMLT(L)=0.0
          END IF
...
!            Ignore changes in SO2 of less than ~10%
            IF (ABS(SO2MLT(L)) < 1E-4) SO2MLT(L)=0.0
          ELSE
            SO2MLT(L)=0.0
          END IF
...
!            Ignore changes in NH3 of less than ~10%
            IF (ABS(NH3MLT(L)) < 1E-4) NH3MLT(L)=0.0
          ELSE
            NH3MLT(L)=0.0
          END IF
...
!            Ignore changes in HNO3 less than ~1%
            IF (ABS(HNOMLT(L)) < 1E-2) HNOMLT(L)=0.0
          ELSE
            HNOMLT(L)=0.0
          END IF
          
          IF (LHDO) THEN
!            HDO mult=1 when no depletion/enhancement of HDO
            HDOMLT(L)=( 1 - HDOFCT )
